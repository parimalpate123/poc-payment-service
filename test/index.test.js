...
describe('Payment Service', () => {
  ...
  it('should get payment status successfully', async () => {
    ...
    mock.onGet(`https://api.payment-gateway.com/status/${paymentId}`).reply(200, {
      id: paymentId,
      status: 'completed',
      amount: 100,
      currency: 'USD',
  ...
  it('should handle payment status retrieval timeout', async () => {
    const paymentId = 'PAY-123';

    mock.onGet(`https://api.payment-gateway.com/status/${paymentId}`).timeout();

    const response = await request(app)
      .get(`/api/v1/payments/${paymentId}`);

    expect(response.status).toBe(500);
    expect(response.body.error).toBe('Failed to fetch payment status');
    expect(response.body.message).toBe('Payment gateway timeout');
  });
});
